...
import { setAgregarSucursales } from "../../../../../redux/slices/EmpresaReducer.ts";

interface ICrearSucursal {
  handleCrearSucursal: () => void;
  initialForm: ICreateSucursal;
  empresa: { id: number };
}

export const CrearSucursal: FC<ICrearSucursal> = ({
  handleCrearSucursal,
  initialForm,
  empresa,
}) => {
  ...
  const { onInputChange, formState } = useForm<ICreateSucursal>(initialForm);
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response: Response = await fetch(
        "http://190.221.207.224:8090/sucursales/create",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ ...formState, idEmpresa: empresa.id }),
        }
      );
